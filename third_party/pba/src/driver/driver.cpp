...
int main(int argc, char* argv[]) 
{
    ...
    // Export input scene to ply format.
    {
      string out_file_name = "Before.ply";
      ...
      outfile << "ply"
        ...
        << std::endl << "element vertex " << point_data.size()
      ...
      {
        const Point3D & pt3D = point_data[i];
        outfile << pt3D.xyz[0] << " " << pt3D.xyz[1] << " " << pt3D.xyz[2]
        << " " << (int)255 << " " << (int)255 << " " << (int)255 << std::endl;
      }
      outfile.close();
    }
    ...
    //pba.SetFocalLengthFixed(true); //if your focal lengths are calibrated. 
    ...
    //WARNING: measurements must be stored in correct order
    //all measurements (in different images) for a single 3D point must be stored continously, 
