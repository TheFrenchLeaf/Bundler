...
#ifdef _MSC_VER
        Float* p = (Float*) _aligned_malloc(size, 16);
        if(p == NULL) throw std::bad_alloc();
        return p;
#else
        char * p = (char*) malloc(size + 16 + 4);
        if(p == NULL) throw std::bad_alloc();
        char * p1 = p + 1;
        char * p2 = (char*) (((((size_t)p1) + 15) >> 4) << 4);
        char * p3 = ( p2 - 1);
        p3[0] = (p2 - p);
        return (Float*) p2;
#endif
...
public:
    ...
    virtual void SetFocalLengthFixed(bool fixed)            {__fixed_focallength = fixed; }
