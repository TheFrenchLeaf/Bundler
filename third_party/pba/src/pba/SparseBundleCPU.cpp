...
#define CPUSBA_USE_SSE
#endif

#ifdef CPUSBA_USE_SSE
#include <xmmintrin.h>
#include <emmintrin.h>
#endif
...
namespace MYSSE
{
    ...
    template<>  class SSE<float>  {
        ...
        static sse_type zero() {return _mm_setzero_ps();}    };
    template<>  class SSE<double> {
        public: typedef  __m128d sse_type;  
        //static size_t   step() {return 2;}
        static sse_type zero() {return _mm_setzero_pd();}    };
...
#ifdef _WIN32
    inline float    sse_sum(__m128 s)    {return s.m128_f32[0] + s.m128_f32[1] + s.m128_f32[2] +s.m128_f32[3];}
    inline double   sse_sum(__m128d s)    {return s.m128d_f64[0] + s.m128d_f64[1];}
#else
    inline float    sse_sum(__m128 s)    {float *f = (float*) (&s); return f[0] + f[1] + f[2] + f[3];}
    ...
    //inline void    sse_store(float *p, __m128 s){_mm_stream_ps(p, s); }
    //inline void    sse_store(double *p, __m128d s)  {_mm_stream_pd(p, s); }
...
#define SSE_T typename SSE<Float>::sse_type
   ...
   inline void SetBlockJtJ(const float * jc, float * block, int radial)
    {
        ...
        {
            __m128 a = _mm_load_ps1(jc);    
            _mm_store_ps(block + 0, _mm_mul_ps(a, j1));
            _mm_store_ps(block + 4, _mm_mul_ps(a, j2));
        }
    }
    inline void AddBlockJtJ(const double * jc, double * block, int radial)
    {
        const int vn = radial ? 8 : 7;
    ...
    inline void SetBlockJtJ(const double * jc, double * block, int radial)
    {
        const int vn = radial ? 8 : 7;
        __m128d j1 = _mm_load_pd(jc);
        __m128d j2 = _mm_load_pd(jc + 2);
        ...
        {
            __m128d a = _mm_load1_pd(jc);    
            _mm_store_pd(block + 0, _mm_mul_pd(a, j1));
            _mm_store_pd(block + 2, _mm_mul_pd(a, j2));
            _mm_store_pd(block + 4, _mm_mul_pd(a, j3));
            _mm_store_pd(block + 6, _mm_mul_pd(a, j4));
        }
    }
   ...
   void ShuffleCameraJacobian(const avec<double>& jc, const vector<int>& map, avec<double>& result)
    {
        ...
        for(size_t i = 0; i < num; ++i)
        {
            ...
            for(int j = 0; j< 16; j += 2) _mm_store_pd(d + j, _mm_loadu_pd(s + j));
        }
    }
    template <class Float>
    ...
    template<class Float>
    void SetBlockJtJ(const Float * jc, Float * block, int radial)
    {
        const int vn = radial ? 8 : 7;
        for(int i = 0; i < vn; ++i)
        {
...
#define WAIT_THREAD(tv, n)      {   for(size_t i = 0; i < size_t(n); ++i) pthread_join(tv[i], NULL) ;}
    ...
    void   ComputeSAXPY(Float a, const avec<Float>& vec1, const avec<Float>& vec2, avec<Float>& result, int mt = 0)
    {
        ...
        if(mt > 1 && result.size() >= mt * 4)
        {
            ...
            {
                size_t first = (result.size() * i / thread_num + 3 ) / 4  * 4 ;
                size_t last_ = (result.size() * (i + 1) / thread_num + 3) / 4 * 4;
    ...
    double ComputeVectorNorm(const avec<Float>& vec, int mt)
    {
        ...
        if(mt > 1 && vec.size() >= mt * 4)
        {
            ...
            for(size_t i = 0; i < thread_num; ++i)
            {
                size_t first = (vec.size() * i / thread_num + 3 ) / 4  * 4 ;
                size_t last_ = (vec.size() * (i + 1) / thread_num + 3) / 4 * 4;
    ...
    template<class Float> 
    inline void JacobianOne(const Float* c, const Float * pt, const Float* ms, Float* jxc, Float* jyc, Float* jxp, Float* jyp,
                    bool focallength_fixed , int radial_distortion)
    {
        const Float* r = c + 4;
...
#endif
				{
					Float jfc =  Float(1.0 + rr1 + rr2); 
					Float ftx = c[0] * (p0_p2 * p0_p2 + p1_p2 * p1_p2);   
					/////////////////////////////////////////////////////
					jxc[0] = focallength_fixed? 0 : p0_p2 * jfc;
					jxc[1] = f_p2_x;
					jxc[2] = 0;
					jxc[3] = -f_p2_x * p0_p2;
					...
					jyc[0] = focallength_fixed? 0 : p1_p2 * jfc;
					jyc[1] = 0;
					jyc[2] = f_p2_y;
					jyc[3] = -f_p2_y * p1_p2;
...
#endif
				{
					jxc[0] = focallength_fixed? 0 : p0_p2;
					jxc[1] = f_p2;
					jxc[2] = 0;
					jxc[3] = -f_p2 * p0_p2;
					jxc[4] = -f_p2 * p0_p2 * y0;
					...
					jyc[0] = focallength_fixed? 0 : p1_p2;
					jyc[1] = 0;
					jyc[2] = f_p2;
					jyc[3] = -f_p2 * p1_p2;
    ...
    DEFINE_THREAD_DATA(ComputeJacobian) 
            ...
            bool focallength_fixed; int radial_distortion; bool shuffle; Float* jct; int i0;  
    BEGIN_THREAD_PROC(ComputeJacobian) 
        ComputeJacobian( q->nproj, q->ncam, q->camera, q->point, q->jc, q->jp,
                    q->jmap, q->sj,  q->ms, q->cmlist, q->focallength_fixed, 
    ...
    void ComputeJacobian(size_t nproj, size_t ncam, const Float* camera, const Float* point, Float*  jc, Float* jp,
                         ...
                         bool focallength_fixed , int radial_distortion, bool shuffle, Float* jct, 
                         int mt = 2, int i0 = 0)
    {
        ...
        if(mt > 1 && nproj >= mt)
        {
            ...
            for(size_t i = 0; i < thread_num; ++i)
            {
                ...
                RUN_THREAD(ComputeJacobian, threads[i],
                    ...
                    focallength_fixed, radial_distortion, shuffle, jct, first);
            }
            WAIT_THREAD(threads, thread_num);
        }else
        {
            ...
            for(size_t i = i0; i < nproj; ++i, jmap += 2, ms += 2, ++cmlist)
            {
                ...
                /////////////////////////////////////////////////////
                JacobianOne(c, pt, ms, jci, jci + 8, jpi, jpi + POINT_ALIGN, focallength_fixed, radial_distortion);
    ...
    void  ComputeDiagonal( const avec<Float>& jcv, const vector<int>& cmapv, const avec<Float>& jpv, const vector<int>& pmapv, 
                        const vector<int>& cmlistv, avec<Float>& jtjdi, bool jc_transpose, int radial)
    {
        //first camera part
        ...
        ///////compute jc part 
        for(size_t i = 0; i < ncam; ++i, jji += 8, ++cmap)
        {
    ...
    void ComputeDiagonalBlockC(size_t ncam,  float lambda1, float lambda2, const Float* jc, const int* cmap,
                const int* cmlist, Float* di, Float* bi, int vn, bool jc_transpose, int mt)
    {
        ...
        if(mt > 1 && ncam >= (size_t) mt)
        {
            ...
            for(size_t i = 0; i < thread_num; ++i)
            {
                ...
                RUN_THREAD(ComputeDiagonalBlockC, threads[i],
                    (last - first), lambda1, lambda2, jc, cmap + first, cmlist,
                     di + 8 * first, bi + bc * first, vn, jc_transpose);
            }
            WAIT_THREAD(threads, thread_num);
        }else
        {
            //avec<Float> buf(64);    Float* pbuf = &buf[0];
            Float bufv[64 + 4]; size_t offset = ((size_t)bufv) & 0xf;
    ...
    void ComputeDiagonalBlock(size_t ncam, size_t npts, float lambda, bool dampd, const Float* jc, const int* cmap,
                const Float*  jp, const int* pmap, const int* cmlist, Float* diag, 
                Float* blocks, int radial_distortion, bool jc_transpose, int mt1 = 2, int mt2 = 2, int mode = 0)
    {
		...
		if(mode == 0)
		{
			...
			////////////////////////////////
			ComputeDiagonalBlockC(ncam, lambda1, lambda2, jc, cmap, cmlist, diag, blocks, vn, jc_transpose, mt1);
    ...
    void ComputeDiagonalBlock_(float lambda, bool dampd, const avec<Float>& camerav,  const avec<Float>& pointv,
                             ...
                             avec<Float>& diag, avec<Float>& blocks, 
							 bool focallength_fixed, int radial_distortion, int mode = 0)
    {
        const int vn = radial_distortion? 8 : 7;
        ...
        //avec<Float> jbuf(24);
        //Float* jxc = &jbuf[0], *jyc= &jbuf[8], *jxp = &jbuf[16], *jyp = &jbuf[20];
        Float jbufv[24 + 4]; size_t offset = ((size_t) jbufv) & 0xf;
		...
		for(size_t i = 0; i < jmapv.size(); i += 2, jmap += 2, ms += 2)
		{
			...
			const Float* c = camera + cidx * 16, * pt = point + pidx * POINT_ALIGN;    
			/////////////////////////////////////////////////////////
			JacobianOne(c, pt, ms, jxc, jyc, jxp, jyp, focallength_fixed, radial_distortion);
    ...
    DEFINE_THREAD_DATA(ComputeJX_) 
           size_t nproj, ncam; const Float* x; Float * jx; 
            ...
            bool focallength_fixed; int radial_distortion; int mode;
    BEGIN_THREAD_PROC(ComputeJX_) 
        ComputeJX_( q->nproj, q->ncam, q->x, q->jx, q->camera, q->point, q->ms, q->sj,
                    q->jmap, q->focallength_fixed, q->radial_distortion, q->mode, 0);
    ...
    void ComputeJX_(size_t nproj, size_t ncam,  const Float* x, Float* jx, const Float* camera,
                    ...
                    bool focallength_fixed, int radial_distortion, int mode, int mt = 16)
    {
        if(mt > 1 && nproj >= mt)
        {
            MYTHREAD threads[THREAD_NUM_MAX];
            ...
            for(size_t i = 0; i < thread_num; ++i)
            {
                ...
                RUN_THREAD(ComputeJX_, threads[i],
                    ...
                    focallength_fixed, radial_distortion, mode);
            }
            WAIT_THREAD(threads, thread_num);
        }else if(mode == 0)
        {
            //avec<Float> jcv(16 + 8); Float* jc = &jcv[0], * jp = &jcv[16];
            Float jcv[24 + 4]; size_t offset = ((size_t) jcv) & 0xf;
            ...
            for(size_t i = 0 ;i < nproj; ++i, ms += 2, jmap += 2, jx += 2)
            {
                ...
                const Float* c = camera + cidx * 16, * pt = point + pidx * POINT_ALIGN;
                /////////////////////////////////////////////////////
                JacobianOne(c, pt, ms, jc, jc + 8, jp, jp + POINT_ALIGN, focallength_fixed, radial_distortion);  
                if(sjc)
                {
                    //jacobian scaling
        ...
        }else if(mode == 1)
        {
            ...
            for(size_t i = 0 ;i < nproj; ++i, ms += 2, jmap += 2, jx += 2)
            {
                ...
                const Float* c = camera + cidx * 16, * pt = point + pidx * POINT_ALIGN;
                /////////////////////////////////////////////////////
                JacobianOne(c, pt, ms, jc, jc + 8, (Float*) NULL, (Float*)NULL, focallength_fixed, radial_distortion);  
        ...
        }else if(mode == 2)
        {
            ...
            for(size_t i = 0 ;i < nproj; ++i, ms += 2, jmap += 2, jx += 2)
            {
                ...
                const Float* c = camera + cidx * 16, * pt = point + pidx * POINT_ALIGN;
                /////////////////////////////////////////////////////
                JacobianOne(c, pt, ms, (Float*) NULL, (Float*) NULL, jp, jp + POINT_ALIGN, focallength_fixed, radial_distortion);  
    ...
    DEFINE_THREAD_DATA(ComputeJtEC_) 
           ...
           const int *jmap, *cmap, *cmlist; bool focallength_fixed; int radial_distortion;
    BEGIN_THREAD_PROC(ComputeJtEC_) 
        ComputeJtEC_(q->ncam, q->ee, q->jte, q->c, q->point, q->ms, q->jmap, q->cmap,
                     q->cmlist, q->focallength_fixed, q->radial_distortion, 0);
    ...
    void ComputeJtEC_(  size_t ncam, const Float* ee,  Float* jte,
                        ...
                        bool focallength_fixed, int radial_distortion, int mt)
    {
        if(mt > 1 && ncam >= mt)
        {
            MYTHREAD threads[THREAD_NUM_MAX];
            ...
            for(size_t i = 0; i < thread_num; ++i)
            {
                ...
                RUN_THREAD(ComputeJtEC_, threads[i],
                    ...
                    cmap + first, cmlist, focallength_fixed, radial_distortion);
            }
            WAIT_THREAD(threads, thread_num);
        ...
        {
            /////////////////////////////////
            Float jcv[16 + 4];          size_t offset = ((size_t) jcv) & 0xf;
            ...
            for(size_t i = 0; i < ncam; ++i, ++cmap, jte += 8, c += 16)
            {
                ...
                for(int j = idx1; j < idx2; ++j)
                {
                    ...
                    JacobianOne(c, pt, ms + index * 2, jcx, jcy, (Float*)NULL, (Float*)NULL, focallength_fixed, radial_distortion);
    ...
    void ComputeJtE_(   size_t nproj, size_t ncam, size_t npt, const Float* ee,  Float* jte,
                        ...
                        bool focallength_fixed, int radial_distortion, int mode, int mt)
    {
        if(mode != 2)
        {
            SetVectorZero(jte, jte + ncam * 8 );
            ComputeJtEC_(ncam, ee, jte, camera, point, ms, jmap, cmap, cmlist, focallength_fixed, radial_distortion, mt);
        }
        if(mode != 1)
        {
                        ...
                        bool focallength_fixed, int radial_distortion, int mode)
    {
        ...
        for(size_t i = 0 ;i < nproj; ++i, jmap += 2, ms += 2, ee += 2)
        {
            ...
            {
                /////////////////////////////////////////////////////
                JacobianOne(c, pt, ms, jc, jc + 8, pj, pj + POINT_ALIGN, focallength_fixed, radial_distortion);
            ...
            {
               /////////////////////////////////////////////////////
                JacobianOne(c, pt, ms, (Float*) NULL, (Float*) NULL, pj, pj + POINT_ALIGN, focallength_fixed, radial_distortion);
...
SparseBundleCPU<Float>:: SparseBundleCPU() : ParallelBA(PBA_INVALID_DEVICE),_num_camera(0), _num_point(0),
                _num_imgpt(0), _num_camera_const(0), _camera_data(NULL), _point_data(NULL), _imgpt_data(NULL),
                _camera_idx(NULL), _point_idx(NULL),  _projection_sse(0)
{
    ...
    if(__verbose_level)			std::cout  << "CPU " << (__cpu_data_precision == 4 ? "single" : "double") 
										   << "-precision solver; " << __num_cpu_cores << " compute cores.\n";
...
//
    ...
    __num_cpu_thread[FUNC_JTEC_JCO] = __num_cpu_cores / 4;
    __num_cpu_thread[FUNC_JTEP] = __num_cpu_cores / 4;
#else
	__multiply_jx_usenoj = false; //seems noj version is slower on my Linux machine.
    __num_cpu_thread[FUNC_JTEC_JCT] = __num_cpu_cores * 2;
	...
	//see the AUTO_MT_NUM macro for definition
...
bool SparseBundleCPU<Float>::InitializeStorageForSFM()
{
    ...
    ALLOCATE_REQUIRED_DATA(_cuImageProj, _num_imgpt, 2);                    //2k
...
bool SparseBundleCPU<Float>::InitializeStorageForCG()
{
    ...
    ALLOCATE_REQUIRED_DATA(_cuVectorJX, _num_imgpt, 2);            //2k
...
void SparseBundleCPU<Float>::EvaluateJacobians()
{
    ...
    if(__jc_store_original || !__jc_store_transpose)
    {
        ...
        ComputeJacobian(_num_imgpt, _num_camera, _cuCameraData.begin(), _cuPointData.begin(), _cuJacobianCamera.begin(), 
            ...
            __fixed_focallength, __use_radial_distortion, false, _cuJacobianCameraT.begin(),
    ...
    }else
    {
        ComputeJacobian(_num_imgpt, _num_camera, _cuCameraData.begin(), _cuPointData.begin(), _cuJacobianCameraT.begin(), 
            ...
            __fixed_focallength, __use_radial_distortion, true, ((Float*) 0), 
    ...
    if(__no_jacobian_store ||  (!__jc_store_original && !__jc_store_transpose))
    {
        if(_cuJacobianPoint.size())
        {
            ProgramCPU::ComputeJtE_(_num_imgpt, _num_camera, _num_point, E.begin(), JtE.begin(), 
                ...
                __fixed_focallength, __use_radial_distortion, mode, __num_cpu_thread[FUNC_JTE_]);
        ...
        }else
        {
            ProgramCPU::ComputeJtE_(_num_imgpt, _num_camera, _num_point, E.begin(), JtE.begin(), 
                _cuCameraData.begin(), _cuPointData.begin(), _cuMeasurements.begin(), &_cuProjectionMap.front(),
                __fixed_focallength, __use_radial_distortion, mode);
...
void SparseBundleCPU<Float>::ComputeJX(VectorF& X, VectorF& JX, int mode)
{
    ...
    if(__no_jacobian_store || (__multiply_jx_usenoj && mode != 2) || !__jc_store_original)
    {
        ProgramCPU::ComputeJX_(_num_imgpt, _num_camera, X.begin(), JX.begin(), _cuCameraData.begin(), _cuPointData.begin(),
                          ...
                          __fixed_focallength, __use_radial_distortion, mode, __num_cpu_thread[FUNC_JX_]);

    }else 
    {
        ProgramCPU::ComputeJX<Float>( _num_imgpt, _num_camera, X.begin(), _cuJacobianCamera.begin(),
...
void SparseBundleCPU<Float>::ComputeBlockPC(float lambda, bool dampd)
{
    ...
    if(__no_jacobian_store ||  (!__jc_store_original && !__jc_store_transpose && __bundle_current_mode != 2))
    {
        ComputeDiagonalBlock_(lambda, dampd, _cuCameraData, _cuPointData, _cuMeasurements, 
            _cuProjectionMap, _cuVectorSJ, _cuVectorJJ, _cuBlockPC,
            __fixed_focallength, __use_radial_distortion,  __bundle_current_mode);
    }else if(__jc_store_transpose)
    {
        ComputeDiagonalBlock(_num_camera, _num_point, lambda, dampd, _cuJacobianCameraT.begin(),
            ...
            &_cuCameraMeasurementList.front(), _cuVectorJJ.begin(), _cuBlockPC.begin(), 
            __use_radial_distortion, true, __num_cpu_thread[FUNC_BCC_JCT], __num_cpu_thread[FUNC_BCP],
    ...
    }else
    {
        ComputeDiagonalBlock(_num_camera, _num_point,lambda, dampd, _cuJacobianCamera.begin(), 
            &_cuCameraMeasurementMap.front(), _cuJacobianPoint.begin(), &_cuPointMeasurementMap.front(),
            &_cuCameraMeasurementList.front(), _cuVectorJJ.begin(), _cuBlockPC.begin(),
            __use_radial_distortion, false,  __num_cpu_thread[FUNC_BCC_JCO], __num_cpu_thread[FUNC_BCP],
			__bundle_current_mode); 
    }
}
...
template<class Float> 
void SparseBundleCPU<Float>::ComputeDiagonal(VectorF& JJI)
{
    ...
    {
        ProgramCPU::ComputeDiagonal(_cuJacobianCameraT, _cuCameraMeasurementMap 
                ,_cuJacobianPoint, _cuPointMeasurementMap, _cuCameraMeasurementList, JJI, true, __use_radial_distortion);
    }else if(__jc_store_original)
    {
        ProgramCPU::ComputeDiagonal(_cuJacobianCamera, _cuCameraMeasurementMap 
                ,_cuJacobianPoint, _cuPointMeasurementMap, _cuCameraMeasurementList, JJI, false, __use_radial_distortion);
    }
}
...
void SparseBundleCPU<Float>::NonlinearOptimizeLM()
{
    ...
    for(int i = 0;    i < __lm_max_iteration  &&  !__abort_flag ; __current_iteration = (++i))
    {
        ...
        //In this library, we check absolute difference instead of relative  difference
