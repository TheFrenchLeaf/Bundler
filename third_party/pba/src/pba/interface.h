        ...
        Float *Uout, Float *Wout, int max_iteration = 150)
{
    ...
    vector<CameraT>        camera_data(num_cameras);    //camera (input/output)
    vector<Point3D>        point_data(num_pts);     //3D point(input/output)
    vector<Point2D>        measurements;   //measurement/projection vector
...
bool run_pba(int num_cameras, int num_pts, int numprojs, Float *motstruct, const Float*imgpts, const char*vmask )
{
    ...
    vector<CameraT>        camera_data(num_cameras);    //camera (input/output)
    vector<Point3D>        point_data(num_pts);     //3D point(input/output)
    vector<Point2D>        measurements(numprojs);   //measurement/projection vector
    vector<int>            camidx, ptidx;  //index of camera/point for each projection
