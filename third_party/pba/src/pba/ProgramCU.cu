...
#define DIAGONAL_SHIFT      0


///////////////////////////////////////////////////////////////
inline void CuTexImage:: BindTexture(textureReference& texRef)
{
    if(_numBytes > MAX_TEXSIZE) fprintf(stderr, "cudaBindTexture: %d > %d\n", _numBytes , MAX_TEXSIZE); 
    cudaError_t e =cudaBindTexture(NULL, &texRef, data(), &texRef.channelDesc, _numBytes);
}
...
inline void ProgramCU::GetBlockConfiguration(unsigned int nblock, unsigned int& bw, unsigned int& bh)
{
    ...
    {
        bh = (nblock + MAX_BLOCKLEN - 1)  / MAX_BLOCKLEN;
        bw = (nblock + bh - 1) / bh;
        bw = ((bw + 31) / 32) * 32;
        bh = (nblock + bw - 1) / bw; 
    }
...
template<bool md, bool pd, bool scaling, bool shuffle> __global__ void jacobian_frt_kernel(
                float4* jc, float4* jp, int nproj, int ptx, int rowsz, float jfc)
{
    ////////////////////////////////
    ...
    if(pd)
    {
        ...
        if(scaling == false)
        {
            if(jc)
            {
				...
				jfc *=  (1 + rr1 + rr2); ft.x *= (p0_p2 * p0_p2 + p1_p2 * p1_p2);  
				jc[jc_pos    ] = make_float4(   p0_p2 * jfc, f_p2_x, 0, -f_p2_x * p0_p2);
				jc[jc_pos + 1] = make_float4(  -f_p2_x * p0_p2 * y0, f_p2_x * (z0 + x0 * p0_p2), -f_p2_x * y0, ft.x * p0_p2);
				jc[jc_pos + 2] = make_float4(   p1_p2 * jfc, 0, f_p2_y, -f_p2 * p1_p2);
				jc[jc_pos + 3] = make_float4(  -f_p2_y * (z0 + y0 * p1_p2), f_p2_y * x0 * p1_p2,  f_p2_y * x0, ft.x * p1_p2);
				JFRT_SET_JC_END
            }
            ////////////////////
    ...
    }else if(md)
    {
        if(scaling == false)
        {
            if(jc)
            {
				...
				float  msn = ms.x * ms.x + ms.y * ms.y; 
				jc[jc_pos    ] = make_float4(   p0_p2 * jfc, f_p2, 0, -f_p2 * p0_p2);
				jc[jc_pos + 1] = make_float4(  -f_p2 * p0_p2 * y0, f_p2 * (z0 + x0 * p0_p2), -f_p2 * y0, -ms.x * msn);
				jc[jc_pos + 2] = make_float4(   p1_p2 * jfc, 0, f_p2, -f_p2 * p1_p2);
        ...
        }else
        {
            if(jc)
            {
				JACOBIAN_SET_JC_BEGIN
				float4 sc1 = tex1Dfetch(tex_jacobian_sj, proj.x);
				jc[jc_pos    ] = make_float4(   p0_p2 * jfc * sc1.x, f_p2 * sc1.y, 0, -f_p2 * p0_p2 * sc1.w);
				jc[jc_pos + 2] = make_float4(   p1_p2 * jfc * sc1.x, 0, f_p2 * sc1.z, -f_p2 * p1_p2 * sc1.w);
...
void ProgramCU::ComputeJacobian(CuTexImage& camera, CuTexImage& point, CuTexImage& jc, 
                                ...
                                bool focallength_fixed , int radial_distortion, bool shuffle)
{
    float jfc = focallength_fixed ? 0.0f : 1.0f;
...
__global__ void update_camera_kernel(int ncam, float4*newcam)
{
    ...
    int index1 = tidx << 1;
    {
        ...
        float4 c2 = make_float4(c1.x + d1.x, c1.y + d1.y, c1.z + d1.z, c1.w + d1.w);
        newcam[index0] = c2;    
    }
    {
        float r[9], dr[9];//, nr[9];
...
template<int VN, int KH, bool JT> __global__ void jtjd_cam_vec32_kernel(
    int num, float* jc, float* jtjd, float* jtjdi)
{
    __shared__ float value[KH * 32]; 
    ...
    if(threadIdx.x < 8)    
    {
        ...
        jtjd[wpos] =  temp;
        jtjdi[wpos] = temp == 0? 0 : 1 / (DIAGONAL_SHIFT + temp);
    }
}
...
void ProgramCU::ComputeDiagonal(CuTexImage& jc, CuTexImage& cmap, CuTexImage& jp,
                            ...
                            CuTexImage& jtjd, CuTexImage& jtjdi, 
                            bool jc_transpose, int radial)
{
    //////////////////////////////////////////////////////////
    size_t szjc = jc.GetDataSize(); 
    ...
    if(jc_transpose)
    {
        if(radial )jtjd_cam_vec32_kernel<8, bheight, true><<<grid1x, block1x>>>(ncam, jc.data(), jtjd.data(), jtjdi.data());
        else       jtjd_cam_vec32_kernel<7, bheight, true><<<grid1x, block1x>>>(ncam, jc.data(), jtjd.data(), jtjdi.data());
    }else
    {
        cmlist.BindTexture(tex_jtjd_cmlist);
        if(radial) jtjd_cam_vec32_kernel<8, bheight, false><<<grid1x, block1x>>>(ncam, jc.data(), jtjd.data(), jtjdi.data()); 
        else       jtjd_cam_vec32_kernel<7, bheight, false><<<grid1x, block1x>>>(ncam, jc.data(), jtjd.data(), jtjdi.data());
    }
...
template<int VN, int KH, bool JT> __global__ void jtjd_cam_block_vec32_kernel(
            int num, float lambda1, float lambda2, float* jc, float* diag, float* blocks)
{
...
void ProgramCU::ComputeDiagonalBlock(float lambda, bool dampd, CuTexImage& jc, CuTexImage& cmap,
          ...
          int radial_distortion, bool jc_transpose, int mode)
{
    size_t szjc = jc.GetDataSize(); 
    unsigned int  ncam = (cmap.GetImgWidth() - 1); //how many cameras
...
template<bool md, bool pd> __device__ void jacobian_internal(
                        ...
                        float jfc, float* jxc, float* jyc, float* jxp, float* jyp)
{
    float m[3];
    float4 ft = tex1Dfetch(tex_jacobian_cam, camera_pos);
    float4 r1 = tex1Dfetch(tex_jacobian_cam, camera_pos + 1);
...
template<bool md, bool pd> __device__ void jacobian_camera_internal(
        int camera_pos, int pt_pos, int tidx, float * r, float jfc, float* jxc, float* jyc)
{
    float m[3];
    float4 ft = tex1Dfetch(tex_jacobian_cam, camera_pos);
    float4 r1 = tex1Dfetch(tex_jacobian_cam, camera_pos + 1);
...
template<bool md, bool pd> __global__ void multiply_jx_noj_kernel(int num, int bwidth, int offset, float jfc, float2* result)
{
    int index = threadIdx.x + blockIdx.x * blockDim.x +  blockIdx.y * bwidth;
    if(index >= num) return;
    ...
    jacobian_internal<md, pd>(proj.x<<1, proj.y, index, data + 9 * threadIdx.x, jfc, jxc, jyc, jxp, jyp);    
...
template<bool md, bool pd> __global__ void multiply_jcx_noj_kernel(int num, int bwidth, float jfc, float2* result)
{
    int index = threadIdx.x + blockIdx.x * blockDim.x +  blockIdx.y * bwidth;
    if(index >= num) return;
    ...
    ////////////////////////////////////////////
    float jxc[8], jyc[8];
    jacobian_camera_internal<md, pd>(proj.x<<1, proj.y, index, data + 9 * threadIdx.x, jfc, jxc, jyc);    
...
void ProgramCU::ComputeJX_(CuTexImage& x,  CuTexImage& jx, CuTexImage& camera, CuTexImage& point, CuTexImage& meas, 
                        CuTexImage& pjmap, bool focallength_fixed, int radial_distortion, int mode)
{
    ...
    float jfc = focallength_fixed ? 0 : 1.0f;
...
template<bool md, bool pd, int KH> __global__ void jte_cam_vec_noj_kernel(int num, int rowsz, float jfc, float* jte)
{
    ...
    for(int i = idx1 + threadIdx.x; i < idx2; i += 32)
    {
        ...
        jacobian_camera_internal<md, pd>(cam << 2, proj.y, index, rp, jfc, jxc, jyc); 
...
void ProgramCU::ComputeJtE_(CuTexImage& e,  CuTexImage& jte, CuTexImage& camera, CuTexImage& point,
                            ...
                            CuTexImage& pjmap, CuTexImage& jp, bool focallength_fixed, int radial_distortion, int mode)
{
...
template<int KH, bool md, bool pd, bool scaling> __global__ void jtjd_cam_block_noj_kernel(
        int num, int rowsz, float lambda1, float lambda2, float jfc,  float* diag, float* blocks)
{
...
void ProgramCU::ComputeDiagonalBlock_(float lambda, bool dampd, CuTexImage& camera, CuTexImage& point,
                        ...
                        CuTexImage& blocks, bool focallength_fixed, int radial_distortion, int mode)
{
    float lambda1 = dampd? 0.0f : lambda;
    float lambda2 = dampd? (1.0f + lambda) : 1.0f;
    float jfc = focallength_fixed? 0.0f : 1.0f;
    ...
    {          
        ...
        if(radial_distortion == -1)   jtjd_cam_block_noj_kernel<bheight1, true, false, true ><<<grid1, block1>>>
                                        (ncam, bw * bheight1, lambda1, lambda2, jfc, diag.data(), blocks.data());
